# 📋 Subscription Descriptions - Copy & Paste Ready

Use these descriptions in App Store Connect for each subscription tier.

---

## 🔥 SPARK TIER

**Display Name (Monthly):** siFia Spark - Monthly  
**Display Name (Annual):** siFia Spark - Annual

**Description:**
```
Ignite your faith journey with:
• Unlimited AI-powered playbooks
• Daily devotionals & Bible verses
• Smart journaling with AI insights
• Prayer tracking & reminders
• Faith points & streak tracking
• Priority support

Perfect for individuals seeking spiritual growth.
```

---

## 🌱 GROWTH TIER

**Display Name (Monthly):** siFia Growth - Monthly  
**Display Name (Annual):** siFia Growth - Annual

**Description:**
```
Accelerate your spiritual growth with:
• Everything in Spark
• Advanced AI coaching & personalization
• Deeper biblical insights
• Enhanced devotional content
• Priority generation queue
• Early access to new features

Ideal for committed believers pursuing transformation.
```

---

## ✨ TRANSFORMATION TIER

**Display Name (Monthly):** siFia Transformation - Monthly  
**Display Name (Annual):** siFia Transformation - Annual

**Description:**
```
Transform your life with Christ:
• Everything in Growth
• Premium AI intelligence
• Personalized spiritual mentoring
• Advanced analytics & insights
• Custom devotional series
• VIP support & coaching

For those seeking profound spiritual transformation.
```

---

## 👨‍👩‍👧‍👦 FAMILY TIER

**Display Name (Monthly):** siFia Family - Monthly  
**Display Name (Annual):** siFia Family - Annual

**Description:**
```
Grow together in faith:
• Everything in Transformation
• Up to 6 family member accounts
• Family devotionals & challenges
• Shared prayer lists
• Family progress tracking
• Dedicated family support

Perfect for families building faith together.
```

---

## 📊 SUBSCRIPTION LEVELS

Set these in App Store Connect:

| Tier | Level |
|------|-------|
| Spark (Monthly & Annual) | 1 |
| Growth (Monthly & Annual) | 2 |
| Transformation (Monthly & Annual) | 3 |
| Family (Monthly & Annual) | 4 |

---

## 💰 PRICING REFERENCE

| Tier | Monthly | Annual |
|------|---------|--------|
| Spark | $9.99 | $99.99 |
| Growth | $19.99 | $199.99 |
| Transformation | $29.99 | $299.99 |
| Family | $39.99 | $399.99 |

---

## ⏱️ FREE TRIAL CONFIGURATION

For ALL subscriptions:
- **Duration:** 3 Days
- **Eligibility:** New Subscribers Only
- **Countries:** All territories

---

## 🔐 PRODUCT IDS (Already Created)

✅ app.sifia.com.spark.monthly  
✅ app.sifia.com.spark.annual  
✅ app.sifia.com.growth.monthly  
✅ app.sifia.com.growth.annual  
✅ app.sifia.com.transformation.monthly  
✅ app.sifia.com.transformation.annual  
✅ app.sifia.com.family.monthly  
✅ app.sifia.com.family.annual

---

## 📝 QUICK WORKFLOW

For each subscription:

1. Click subscription in App Store Connect
2. Go to "Subscription Localizations"
3. Add/Edit English (U.S.)
4. Copy display name from above
5. Copy description from above
6. Set subscription level (1-4)
7. Add 3-day free trial
8. Set price
9. Save

Repeat for all 8 subscriptions!

---

**Time per subscription:** ~5 minutes  
**Total time:** ~40 minutes for all 8
