...
import PrayerResponseSheet, { type PrayerResponseOperations } from './PrayerResponseSheet';
...
import { formatPrayerDateLong } from '../../utils/date';
...
type Props = { prayer: PrayerApiEntry; onClose: () => void; onSave: (data: PrayerChanges) => Promise<void>; onDelete?: () => Promise<void>; onPrayNow?: (prayer: PrayerApiEntry) => void; responseOperations?: PrayerResponseOperations; renderUpdate: (prayer: PrayerApiEntry, save: (data: PrayerChanges) => Promise<void>, close: () => void) => React.ReactNode };
export default function PrayerDetails({ prayer, onClose, onSave, onDelete, onPrayNow, responseOperations, renderUpdate }: Props) {
  ...
  const orientation = getPrayerTypePresentation(current);
  const isUnprayedRequest = current.is_prayer_request === true && current.prayed !== true;
  ...
  return <Modal visible transparent presentationStyle="overFullScreen" animationType="none" onRequestClose={closeDetails}>
    <Animated.View style={[styles.page, { opacity: pageExit, transform: [{ translateY: pageExit.interpolate({ inputRange: [0, 1], outputRange: [8, 0] }) }, { scale: pageExit.interpolate({ inputRange: [0, 1], outputRange: [0.99, 1] }) }] }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 18, paddingBottom: insets.bottom + 24 }}>
        <StepFadeIn delay={70}><View style={styles.card}>
          <View style={styles.orientation}>
            {orientation.detailLabel !== 'Prayer' && <ThemedText weight="semiBold" style={styles.orientationTitle}>{orientation.detailLabel}</ThemedText>}
            {!!orientation.detailContext && <ThemedText style={styles.orientationContext}>{orientation.detailContext}</ThemedText>}
            {!!orientation.originLabel && <ThemedText style={styles.orientationOrigin}>{orientation.originLabel}</ThemedText>}
            {!!formatPrayerDateLong(current.created_at) && <ThemedText style={styles.orientationOrigin}>{isUnprayedRequest ? 'Saved' : 'Written'} {formatPrayerDateLong(current.created_at)}</ThemedText>}
            {!!orientation.requestContext && <View style={styles.request}><View style={styles.requestContent}><Ionicons name="mail-unread-outline" size={12} color={Colors.alertCoral} /><ThemedText style={styles.hint}>Original request: {orientation.requestContext}</ThemedText></View></View>}
          </View>
          <View style={styles.row}>{label(isUnprayedRequest ? 'ORIGINAL REQUEST' : 'YOUR PRAYER')}{!isUnprayedRequest && <StepFadeIn delay={120}><TouchableOpacity style={styles.iconAction} disabled={saving} activeOpacity={0.75} onPress={() => { triggerLightHaptic(); setDraft(current.content); setEditor('prayer'); }} accessibilityLabel="Edit prayer"><Pencil size={17} color={Colors.sage} strokeWidth={1.8} /></TouchableOpacity></StepFadeIn>}</View>
        ...
        <StepFadeIn delay={140}><View style={styles.card}>
          <View style={styles.row}>{label('PRAYER STATUS')}</View>
          ...
          <View style={styles.actions}>{action(current.is_prayer_request ? 'Pray now' : 'I prayed for this today', 'prayer-hands', () => { if (current.is_prayer_request && onPrayNow) { onPrayNow(current); return; } if (current.is_prayer_request) { setEditor('response'); return; } void run({ ...base(), prayed: true, prayer_count: (current.prayer_count ?? (current.prayed ? 1 : 0)) + 1, last_prayed_at: new Date().toISOString() }); }, false, 210)}{tracked && needs.length < 2 && action('Record an answer', 'sparkle', () => markAnswer(), false, 255)}</View>
      ...
      {editor === 'response' && <PrayerResponseSheet request={current} operations={responseOperations} onClose={() => setEditor(null)} onSaved={() => { setEditor(null); closeDetails(); }} />}
