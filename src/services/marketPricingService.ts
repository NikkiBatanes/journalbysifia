...
export class MarketPricingService {
   ...
   */
  async getPricingVariant(userId: string, market: Market): Promise<'control' | 'variant'> {
   ...
   */
  getABTestPricing(market: Market, variant: 'control' | 'variant'): Record<SubscriptionTier, MarketPricing> {
    const basePricing = this.getAllPricing(market);
