import {
  classifyReflection,
  getReflectionReviewText,
  getReviewCapture,
} from '../reviewCaptureService';
...
it.each([
  ...
  'classifies legitimate plain text without emitting a JSON parse error',
  content => {
    const error = jest
      .spyOn(Logger, 'error')
      .mockImplementation(() => undefined);
    expect(classifyReflection({...base, content})).toMatchObject({
      text: content,
      kind: 'morning',
    });
...
it('supports JSON objects and arrays', () => {
  ...
  expect(getReflectionReviewText('["Righteous","Faithful"]')).toBe(
    'Righteous · Faithful',
  );
  expect(
    getReflectionReviewText(
      '{"text":"A direct reflection","blocks":[{"text":"ignored"}]}',
    ),
  ).toBe('A direct reflection');
});
...
it('preserves Weekly Review capture classification for plain-text Scripture', async () => {
  (getLocalReflections as jest.Mock).mockImplementation(async (type: string) =>
    type === 'scripture'
      ? [
          {
            ...base,
            title: 'Psalm 1',
            content: 'Righteous · Knows His people',
            metadata: {psalmRead: true},
  ...
  expect(capture.items[0]).toMatchObject({
    ...
    kind: 'morning',
    title: 'Psalm 1',
    text: 'Righteous · Knows His people',
    passageRead: true,
  });
  ...
  expect(
    classifyReflection({
      ...
      metadata: {
        proverbRead: true,
        selectedWisdom: [
          {id: 'guard', label: 'Guard your heart', verses: 'Proverbs 4:23'},
        ],
        wisdomApplications: {
          guard: 'I need stronger boundaries around what I consume.',
        },
    ...
    wisdomItems: [
      {
        ...
        response: 'I need stronger boundaries around what I consume.',
      },
...
it('retains distinct presentation types for thoughts, guided reflections, Bible Study, and session notes', () => {
  expect(
    classifyReflection({
      ...
      selected_date: '2026-09-17',
      metadata: {journalClassification: 'thoughts'},
    }),
  ).toMatchObject({
    kind: 'reflection',
  ...
  expect(
    classifyReflection({
      ...
      content: JSON.stringify({
        ...
        pathId: 'trust',
        pathTitle: 'Trust',
        currentStepId: 'notice',
        completed: true,
        answers: [{stepId: 'notice', text: 'Release the outcome.', notes: []}],
      }),
    }),
  ).toMatchObject({
    kind: 'reflection',
  ...
  expect(
    classifyReflection({
      ...
      title: 'Where can you receive rest?',
      content: 'I can stop measuring the day by output.',
      selected_date: '2026-09-17',
    }),
  ).toMatchObject({
    kind: 'reflection',
  ...
  expect(
    classifyReflection({
      ...
      content: 'A deadline I cannot control.',
      selected_date: '2026-09-17',
    }),
  ).toMatchObject({
    kind: 'reflection',
  ...
  expect(
    classifyReflection({
      ...
      type: 'guided',
      source: 'guided',
      title: 'Where have I noticed God at work?',
      content: 'In the clarity that came after I stopped forcing an answer.',
      selected_date: '2026-09-17',
      metadata: {
        prompt: 'Where have I noticed God at work?',
        questionTopic: 'With God',
      },
    }),
  ).toMatchObject({
    kind: 'reflection',
    ...
    subtitle: 'Guided prompt',
    lifeArea: 'With God',
    text: 'In the clarity that came after I stopped forcing an answer.',
  });
  expect(
    classifyReflection({
      id: 'devotional',
      ...
      source: 'devotional',
      title: 'Remain in Me',
      content: 'Fruitfulness grows from staying close to Jesus.',
      selected_date: '2026-09-17',
      metadata: {devotional_title: 'Abide', day_title: 'Remain'},
    }),
  ).toMatchObject({
    kind: 'reflection',
  ...
  expect(
    classifyReflection({
      ...
      title: 'James 1:2–8',
      selected_date: '2026-09-17',
      metadata: {passage: {translation: 'NASB'}},
      content: JSON.stringify({
        format: 'bible_study_v1',
        highlights: [],
        observation: {text: 'God welcomes my request for wisdom.'},
        understanding: {text: ''},
        response: {text: ''},
        prayer: {text: '', saveToPrayerJournal: false},
      }),
    }),
  ).toMatchObject({
    kind: 'scripture',
    ...
    detail: 'NASB',
    text: 'God welcomes my request for wisdom.',
  });
  expect(
    classifyReflection({
      id: 'meeting',
      ...
      selected_date: '2026-09-17',
      content: JSON.stringify({
        blocks: [{kind: 'key', text: 'Choose the next faithful priority.'}],
      }),
      metadata: {
        sessionNoteType: 'meeting',
        sessionNoteDetails: {
          meeting: {person: 'Product team', event: 'Weekly planning'},
        },
      },
    }),
  ).toMatchObject({
  ...
  const journalBlocks = [
    {
      id: 'photo',
      kind: 'photo' as const,
      text: 'A meaningful afternoon',
      uri: 'file:///journal/photo.jpg',
    },
    {
      id: 'table',
      kind: 'table' as const,
...
it('preserves Scripture Note blocks and Bible version for the shared Weekly Review card', () => {
  const journalBlocks = [
    {id: 'key', kind: 'key' as const, text: 'Grace meets me in weakness.'},
    {id: 'response', kind: 'response' as const, text: 'Ask God for help.'},
  ];
  ...
  expect(
    classifyReflection({
      ...
      content: 'I do not need to hide my limits.',
      selected_date: '2026-09-17',
      metadata: {version: 'NIV', journalBlocks},
    }),
  ).toMatchObject({
    kind: 'scripture',
...
it('reads modern Gratitude lists and Today Win payloads without flattening them into generic Journal cards', async () => {
    ...
    async (type: string) => {
      if (type === 'gratitude') {
        return {
          ...
          content: JSON.stringify({
            items: [
              {id: '1', text: 'A quiet morning'},
              {id: '2', text: 'Timely help'},
            ],
          }),
        };
      }
      if (type === 'today_win') {
        return {
          ...
          selected_date: '2026-09-17',
          content: JSON.stringify({
            winType: 'faithfulness',
            winTypeName: 'Faithfulness',
            quietWin: 'I sent the proposal even though I felt uncertain.',
          }),
        };
      }
      ...
      if (type === 'looking_forward') {
        return {
          ...
          selected_date: '2026-09-17',
          content: JSON.stringify({
            entry: {text: 'A quiet dinner together'},
            emotionName: 'Hopeful',
            emotionIcon: 'heart',
          }),
        };
      ...
      if (type === 'morning_check_in') {
        return {
          ...
          content: JSON.stringify({
            ...
            underneathIt: 'I am trusting God with the outcome.',
  ...
  (getLocalJournalEntries as jest.Mock).mockImplementation(
    async (type: string) =>
      type === 'todo'
        ? [
            {
  ...
  (getScripturePassage as jest.Mock).mockResolvedValue({
  ...
  expect(capture.items.find(item => item.id === 'gratitude')).toMatchObject({
    ...
    lines: ['A quiet morning', 'Timely help'],
  });
  expect(capture.items.find(item => item.id === 'win')).toMatchObject({
    kind: 'win',
    presentation: 'today_win',
    detail: 'Faithfulness',
    text: 'I sent the proposal even though I felt uncertain.',
  });
  expect(capture.items.find(item => item.id === 'focus')).toMatchObject({
    kind: 'journal',
    presentation: 'focus',
    title: 'Be present',
    text: 'Listen before responding.',
    focusIcon: 'hands-pray',
    focusIconType: 'material',
    completedPriorityCount: 2,
    focusPriorities: [
      {text: 'Listen well', completed: true},
      {text: 'Call Mom', completed: true},
      {text: 'Rest', completed: false},
  ...
  expect(
    capture.items.find(item => item.id === 'looking-forward'),
  ).toMatchObject({
    kind: 'journal',
    presentation: 'looking_forward',
    title: 'A quiet dinner together',
    detail: 'Hopeful',
    feelingIcon: 'heart',
  ...
  expect(capture.items.find(item => item.id === 'check-in')).toMatchObject({
    ...
    title: 'Hopeful',
    text: 'I am trusting God with the outcome.',
    detail: 'Isaiah 41:10',
    scriptureText: 'Do not fear, for I am with you.',
    feelingIcon: 'sunny-outline',
    feelingIconType: 'ionicons',
  });
  expect(getScripturePassage).toHaveBeenCalledWith('Isaiah 41:10', 'NASB');
  expect(capture.items.find(item => item.id === 'done-todo')).toMatchObject({
    kind: 'journal',
    presentation: 'todo',
    completed: true,
    priority: true,
  });
  expect(capture.items.find(item => item.id === 'pending-todo')).toMatchObject({
    kind: 'journal',
    presentation: 'todo',
    completed: false,
  });
