...
export const REVIEW_ACTIVITY_GROUPS = [
  ...
  },
  {
    ...
    icon: 'sunny-outline',
    color: '#C6A36D',
    presentations: ['gratitude_list', 'today_win'],
  },
  {
    key: 'everyday',
    label: 'Everyday life',
    icon: 'leaf-outline',
    color: '#A4AA92',
    presentations: ['morning_check_in', 'focus', 'todo', 'looking_forward'],
  },
] as const;

export const NEEDS_CARE_COLOR = '#985D54';
export const NEEDS_CARE_BACKGROUND_COLOR = '#F5E5E0';

export const LIFE_RATINGS: Record<
  string,
...
> = {
  ...
  struggling: {
    label: 'Needs care',
    color: NEEDS_CARE_COLOR,
    background: NEEDS_CARE_BACKGROUND_COLOR,
  },
};
...
export const buildWeeklyReviewSummary = (
  review: Pick<
    LocalReviewEntry,
    'periodStart' | 'periodEnd' | 'answers' | 'memorableItems'
  >,
  ...
  const moments = (capture?.items ?? []).filter(item => {
    ...
    if (
      item.selectedDate < review.periodStart ||
      item.selectedDate > review.periodEnd ||
      seen.has(key)
    ) {
      return false;
    }
  ...
  while (
    ...
    days.push({
      date,
      label: cursor.toLocaleDateString('en', {weekday: 'short'}),
      day: cursor.getDate(),
  ...
  const lifeAreas = WEEKLY_LIFE_AREAS.map(area => ({
    ...area,
    value: review.answers[area.answerKey]?.trim() ?? '',
  }));
  return {
    moments,
    days,
    ...
    categories: REVIEW_ACTIVITY_GROUPS.map(group => ({
      ...group,
      count: moments.filter(item =>
        (group.presentations as readonly string[]).includes(item.presentation),
      ).length,
    })).filter(group => group.count > 0),
    lifeAreas,
    lifeCounts: ['well', 'okay', 'struggling'].map(value => ({
      value,
      count: lifeAreas.filter(area => area.value === value).length,
    })),
