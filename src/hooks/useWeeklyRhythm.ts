...
export const useWeeklyRhythm = (start: string, end: string) => {
  ...
  useFocusEffect(useCallback(() => {
    ...
    const load = async () => {
      ...
      try {
        const rhythm = await getWeeklyRhythm(start, end);
        if (active && request === version) setResult({ key: `${start}:${end}`, rhythm });
      } catch {
