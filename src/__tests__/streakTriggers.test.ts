...
describe('Unified tracker coverage', () => {
  ...
  it('shows a compact tracker with a full tracker destination', () => {
    ...
    expect(card).toContain("import {Flame} from 'lucide-react-native'");
    expect(card).toContain("import PrayerHandsIcon from '../common/PrayerHandsIcon'");
    expect(card).toContain('<Flame size={24} color={streakAccentColor}');
    expect(card).toContain('<PrayerHandsIcon size={20} color={Colors.sage} strokeWidth={2.2}');
    expect(card).toContain("variant === 'profile'");
    expect(card).toContain('? Colors.alertCoral');
    expect(card).toContain(': Colors.faithGold');
    expect(card).toContain('color={streakAccentColor}');
    expect(card).toContain('{color: streakAccentColor}');
    ...
    expect(card).toContain("returnToMore ? {returnTo: 'More'} : undefined");
    ...
    expect(screen).toContain('paddingTop: insets.top + 8');
