...
jest.mock('@react-native-async-storage/async-storage', () => ({
  ...
  removeItem: jest.fn(async (key: string) => mockData.delete(key)),
}));
jest.mock('../../../services/reviewCaptureService', () => ({
  getReviewCapture: jest.fn(async (start: string, end: string) => ({
    periodStart: start,
    periodEnd: end,
    items: [],
    summary: {
...
jest.mock('../../../storage/reviewStorage', () => ({
  getLocalReviewForPeriod: jest.fn(async () => null),
  deleteLocalReview: jest.fn(async () => undefined),
}));
...
describe('Review QA loader isolation', () => {
  ...
  it('loads canonical routine and direct Heart Journal data for the Monday-start Weekly period', async () => {
    ...
    expect(mondayPsalm).toMatchObject({
      ...
      metadata: {
        psalmNumber: 1,
        selectedAttributes: ['Righteous', 'Knows His people'],
      },
    });
    ...
    expect(sundayPsalm).toMatchObject({
      ...
      metadata: {
        psalmNumber: 7,
        selectedAttributes: ['Refuge', 'Righteous Judge'],
      },
    });
    ...
    expect(fridayProverb).toMatchObject({
      title: 'Proverbs 5',
      metadata: {proverbNumber: 5, proverbRead: false},
    });
    ...
    expect(
      scriptureNotes.every(
        entry =>
          entry.type === 'scripture' && entry.source === 'scripture_note',
      ),
    ).toBe(true);
    ...
    expect(
      heartJournalEntries.filter(
        entry => entry.metadata.journalClassification === 'thoughts',
      ),
    ).toHaveLength(5);
    expect(
      heartJournalEntries.filter(
        entry => entry.metadata.journalClassification === 'notes',
      ),
    ).toHaveLength(3);
    expect(
      heartJournalEntries.every(
        entry => entry.type === 'free' && entry.source === 'freeform',
      ),
    ).toBe(true);
    expect(
      heartJournalEntries.some(
        entry =>
          entry.type === 'guided' ||
          entry.source === 'guided' ||
    ...
    const structuredEntries = heartJournalEntries.filter(
      entry => entry.metadata.journalBlocks?.length,
    );
    expect(structuredEntries).toHaveLength(3);
    expect(
      new Set(
        structuredEntries.map(entry => entry.metadata.journalClassification),
      ),
    ).toEqual(new Set(['notes', 'thoughts', 'brain_dump']));
    expect(
      new Set(
        structuredEntries.flatMap(entry =>
          entry.metadata.journalBlocks.map((block: any) => block.kind),
        ),
      ),
    ).toEqual(
      new Set([
        'text',
        ...REFLECTION_NOTE_TYPES.filter(item => item.kind !== 'column').map(
          item => item.kind,
        ),
      ]),
    );
    expect(
      structuredEntries
        .flatMap(entry => entry.metadata.journalBlocks)
        .find((block: any) => block.kind === 'photo')?.uri,
    ).toBeTruthy();
    expect(
      structuredEntries
        .flatMap(entry => entry.metadata.journalBlocks)
        .find((block: any) => block.kind === 'voice')?.uri,
    ).toBeTruthy();
    ...
    const structuredGuidedEntries = guidedEntries.filter(
      entry => entry.metadata.guidedJourney !== undefined,
    );
    expect(chosenQuestionEntries).toHaveLength(4);
    expect(
      chosenQuestionEntries.every(
        entry =>
          entry.type === 'guided' &&
          entry.source === 'guided' &&
          entry.title === entry.metadata.prompt &&
          GUIDED_PROMPTS.includes(entry.metadata.prompt) &&
          ['With God', 'My Heart', 'Health', 'Rest & Rhythms'].includes(
            entry.metadata.questionTopic,
          ) &&
    ...
    expect(
      chosenQuestionEntries.some(entry => entry.source === 'guided_prompt'),
    ).toBe(false);
    expect(
      chosenQuestionEntries.map(entry => entry.metadata.questionTopic),
    ).toEqual(['With God', 'My Heart', 'Health', 'Rest & Rhythms']);
    expect(structuredGuidedEntries).toHaveLength(10);
    expect(
      structuredGuidedEntries.every(
        entry =>
          entry.id.startsWith('dev-review-v2:weekly:guided-reflection:') &&
          entry.type === 'guided' &&
          entry.source === 'guided' &&
          parseGuidedReflection(entry.content)?.pathId ===
            entry.metadata.guidedJourney.pathId &&
          entry.metadata.journalBlocks.length ===
            entry.metadata.guidedJourney.answers.flatMap(
              (answer: any) => answer.notes,
            ).length,
      ),
    ).toBe(true);
    expect(
      new Set(
        structuredGuidedEntries.map(
          entry => entry.metadata.guidedJourney.pathId,
        ),
      ),
    ).toEqual(
      new Set([
    ...
    expect(
      new Set(
        structuredGuidedEntries.flatMap(entry =>
          entry.metadata.journalBlocks.map((block: any) => block.kind),
        ),
      ),
    ).toEqual(
  ...
  it('loads August Monthly activity and exposes Monthly-only eligibility', async () => {
    const result = await loadReviewQAScenario('monthly');
    expect(result.review).toBeNull();
    expect(
      [...mockData.keys()].filter(key =>
        key.startsWith('routine_state:morning:'),
    ...
    expect(JSON.parse(mockData.get('journal:gospel:share-events:v1')!)).toEqual(
      [
        expect.objectContaining({
          id: 'dev-review-v2:monthly:gospel-share:1',
  ...
  it('automatically purges a previously seeded QA manifest and its records', async () => {
    mockData.set(
      ...
      JSON.stringify({
        keys: ['journal_local:gratitude:2026-09-16:dev-review-v2:old'],
        protectedReviewIds: [],
        periods: [],
        scenarioId: 'weekly',
        referenceDate: '2026-09-21',
    ...
    mockData.set(
      'journal_local:gratitude:2026-09-16:dev-review-v2:old',
      '{"id":"dev-review-v2:old"}',
    );
    await expect(getActiveReviewQAContext()).resolves.toBeNull();
    expect(
      mockData.has('journal_local:gratitude:2026-09-16:dev-review-v2:old'),
    ).toBe(false);
  ...
  it('clears QA keys while preserving normal Journal, Prayer, Review, Bible, Gospel, and Prayer V2 data', async () => {
    const protectedKeys = [
      'journal_local:gratitude:2026-09-16:user',
      'prayer_local:2026-09-16:user',
      'review_local:weekly:user',
      'bible_study_session:user',
      'gospel:user',
      'prayer_local:2026-09-16:dev-prayer-v2:fixture',
      'routine_state:morning:2026-09-16',
      'journal_local_singleton:morning_check_in:2026-09-16',
    ];
    ...
    expect(
      [...mockData.keys()].some(key => key.includes('dev-review-v2:')),
    ).toBe(false);
  });
  it('isolates and restores a saved Review without overwriting its content', async () => {
    ...
    await expect(loadReviewQAScenario('weekly')).resolves.toMatchObject({
      review: null,
    });
    const manifest = JSON.parse(mockData.get('dev-review-v2:manifest')!);
    expect(manifest.hiddenReviews).toEqual([saved]);
    await clearReviewQAData();
    expect(JSON.parse(mockData.get('review_local:weekly:real')!)).toEqual(
      saved,
    );
  });
  it('temporarily hides an empty legacy draft so the ready QA card can render', async () => {
    ...
    const {deleteLocalReview} = jest.requireMock(
