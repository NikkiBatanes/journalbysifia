...
import { normalizeTruthScreenEnhancement, type TruthScreenEnhancement } from '../_shared/truthScreenEnhancement.ts';
  ...
  schema: {
    ...
    properties: {
      ...
      cover: {
        ...
        required: ['title', 'subtitle', 'estimated_minutes'],
...
function normalizeEstimatedMinutes(value: unknown, fallback = 12): number {
  const minutes = Number(value);
  if (!Number.isFinite(minutes)) return fallback;
  return Math.max(5, Math.min(30, Math.round(minutes)));
}
...
function splitIntoSentences(text: string): string[] {
  ...
  if (typeof Segmenter === 'function') {
    ...
    return Array.from(segmenter.segment(normalized), (part: any) => String(part.segment || '').trim())
      .filter(Boolean);
  }
...
function paginateTruthInLove(text: string): TruthBeat[] {
  const paragraphs = String(text || '')
    .split(/\r?\n[ \t]*\r?\n(?:[ \t]*\r?\n)*/)
    .map(paragraph => paragraph.trim())
    .filter(Boolean);

  // The test prompt uses one paragraph per semantic screen. Preserve those
  // boundaries even when one paragraph is substantially longer than another.
  if (paragraphs.length >= 2 && paragraphs.length <= 5) {
    return paragraphs.map(paragraph => {
      const sentences = splitIntoSentences(paragraph);
      return {
        label: '',
        primaryTruth: sentences[0] || paragraph,
        supportingTruth: sentences.slice(1).join(' ') || undefined,
        presentation: 'statement' as const,
      };
    });
  }

  // Older generations and safety overrides retain the balanced-page fallback.
  const sentences = paragraphs.flatMap(paragraph => splitIntoSentences(paragraph));

  if (sentences.length === 0) return [];

  const pageCount = Math.min(5, sentences.length);
  const remaining = [...sentences];
  const pages: string[][] = [];

  while (pages.length < pageCount) {
    const groupsLeft = pageCount - pages.length;
    const wordsLeft = remaining.reduce((total, sentence) => total + countWords(sentence), 0);
    const targetWords = wordsLeft / groupsLeft;
    const page: string[] = [];
    let pageWords = 0;

    while (remaining.length > groupsLeft - 1) {
      const next = remaining[0];
      const nextWords = countWords(next);
      if (
        page.length > 0 &&
        Math.abs(pageWords - targetWords) <= Math.abs(pageWords + nextWords - targetWords)
      ) {
        break;
      }
  ...
  return pages.map(page => ({
    label: '',
    primaryTruth: page[0],
    supportingTruth: page.slice(1).join(' ') || undefined,
    presentation: 'statement',
  }));
}
...
function validatePlaybook(json: Record<string, any>, originalInput = ''): ValidationResult {
  ...
  if (!json.cover || typeof json.cover !== 'object') {
    softIssues.push('cover is missing — will auto-repair');
  } else {
    if (!String(json.cover.title || '').trim()) {
      softIssues.push('cover.title is missing — will auto-repair');
    }
    if (!String(json.cover.subtitle || '').trim()) {
      softIssues.push('cover.subtitle is missing — will auto-repair');
    }
  }
  ...
  if (json.truth_summary) {
    ...
    const openingSentence = getOpeningSentence(summaryText);
    const openingWordCount = countWords(openingSentence);
    if (wordCount > 90) {
      softIssues.push(`truth_summary is too long (${wordCount} words — expected a concrete Christ-centered orientation followed by supporting text, at most 90 words total)`);
    }
    if (openingWordCount > 22) {
      softIssues.push(`truth_summary primary sentence is too long (${openingWordCount} words — keep the opening orientation to about 10-22 words and move explanation into supporting sentences)`);
    }
    if (summaryText.trim() === openingSentence.trim()) {
      softIssues.push('truth_summary needs supporting sentences after its short opening orientation; ground the user in Christ within their actual responsibility, releasing burdens that do not belong to them only where relevant, without giving the whole answer yet');
    }
  }
  ...
  if (json.truth_in_love) {
    ...
    if (openingWordCount > TRUTH_IN_LOVE_LONG_OPENING_WORDS) {
      softIssues.push(`Truth in Love opening is too long (${openingWordCount} words). Begin concisely with the discernment function this situation needs first; keep triggered safety and doctrinal guidance first.`);
    }
    if (INDIRECT_TRUTH_OPENERS.some(regex => regex.test(openingSentence))) {
      softIssues.push(`Truth in Love opening is indirect ("${openingSentence.slice(0, 90)}"). Start concretely with the function the situation needs, preserving triggered safety and doctrinal guidance.`);
    }
  }
...
function repairPlaybook(json: Record<string, any>, originalInput = ''): Record<string, any> {
  ...
  const existingCover = repaired.cover && typeof repaired.cover === 'object' ? repaired.cover : {};
  repaired.cover = {
    title: cleanMarkdown(String(existingCover.title || repaired.playbook_title || 'Your Guided Playbook')),
    subtitle: cleanMarkdown(String(existingCover.subtitle || 'Biblical clarity and faithful next steps for what you are facing.')),
    estimated_minutes: normalizeEstimatedMinutes(existingCover.estimated_minutes, 12),
  };
...
function parseJsonPlaybook(
  ...
  const rawCover = json.cover && typeof json.cover === 'object' ? json.cover : {};
  ...
  const playbook: Playbook = {
    ...
    subtitle: '',
    cover: {
      title: cleanMarkdown(String(rawCover.title || json.playbook_title || '')),
      subtitle: cleanMarkdown(String(rawCover.subtitle || '')),
      estimatedMinutes: normalizeEstimatedMinutes(rawCover.estimated_minutes, 12),
    },
  ...
  if (sourceParagraphs.length === playbook.truthInLove.beats.length) {
    playbook.truthInLove.beats.forEach((beat, index) => {
      beat.enhancement = enhancementFor(sourceParagraphs[index]);
    });
  }
...
function sanitizePlaybook(playbook: Playbook) {
  ...
  if (playbook.cover) {
    playbook.cover = {
      title: sanitizeText(playbook.cover.title),
      subtitle: sanitizeText(playbook.cover.subtitle),
      estimatedMinutes: normalizeEstimatedMinutes(playbook.cover.estimatedMinutes, 12),
    };
  }
  ...
  try {
    ...
    if (architecturalIssues.length > 0) {
      ...
      const correctionNote = [
        ...
        '  For ordinary playbooks, use 2-5 blank-line-separated truth_in_love paragraphs, giving 3-6 screens including truth_summary. Choose only needed functions: reality, heart, biblical tension, decision separation, and immediate response. Omit, combine, or reorder them to fit the situation; never invent a function to preserve a sequence. Finish with a proportionate immediate response.',
        '  Preserve semantic paragraph boundaries and distinguish known facts from assumptions. Triggered safety and doctrinal modules take priority over this flow.',
