...
function directChallengeToSave(result: any): Record<string, unknown> {
  ...
  if (result.cover?.title && result.cover?.subtitle) base.cover = result.cover;
  return base;
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const generateReplacement = async (extraInstruction = '') => {
      const generationResponse = await fetch(`${supabaseUrl}/functions/v1/generate-guided-playbook-v146test`, {
        method: 'POST',
        headers: {
    ...
    if (truthInLoveIsTooShort(generated)) {
      ...
      const retry = await generateReplacement([
        'RETRY QUALITY FIX:',
        'The previous refinement made the complete Truth in Love response too short.',
        'Regenerate the full playbook with one complete 2-4 paragraph Truth in Love diagnosis.',
        'Keep it continuous, specific, and grounded in the original moment.',
        'The complete truth_in_love field must contain at least 280 characters.',
      ].join('\n'));
